...
涉及的产物（跨步骤汇总）：`bug-investigation.md`（步骤 3）、`code-review-*.md`（步骤 6.1）、`integration-test-plan.md`（步骤 6.2）、`integration-test-report.md`（步骤 6.4）。详细映射 / 同步流程 / STATUS.md 记账格式 / 通知 URL 规范全部归 `tripo-task-dirs`，本 skill 不复述。

...
class TestShouldRespond:
    ...
    def test_responds_to_group_at_bot(self):
        assert should_respond({"chat_type": "group", "content": "@_user_1 帮我查一下", "sender_type": "user"}) is True
...
class TestHandleMessage:
    ...
    def test_cleans_at_mention_from_prompt(self, mock_add, mock_remove, mock_reply):
        ...
        client.receive_response = fake_response
        run_async(handle_message(client, self._event(content="@_user_1 帮我查一下", chat_type="group")))

        prompt = client.query.call_args[0][0]
        assert "@_user_1" not in prompt

...
- `src/config.py` (`ROOT`, `CONFIG`, `PERSONA`, `OWNER_ID`): 配置加载模块，将 `persona.md` 内容读入 `PERSONA` 常量供注入使用。
...
- **2. SDK 初始化:** `src/main.py:26-32` 构造 `ClaudeAgentOptions`，设置 `system_prompt=SystemPromptPreset(append=PERSONA)`，`setting_sources=["project"]`，`permission_mode="bypassPermissions"`，`can_use_tool=permission_gate`。
...
### 4.2 CLAUDE.md 与 persona.md 的分工
...
两者共同构成完整的 Agent 行为规范：前者定义"Agent 能做什么"，后者定义"Agent 以什么身份和风格做"。

...
| 文档 | 说明 |
|------|------|
| [event-pipeline.md](architecture/event-pipeline.md) | 事件驱动管道、query/response 解耦模型（send_message + session_reader）、prompt 构建（_build_prompt: 角色·名字·ID·场景上下文）、富消息预处理、per-session FIFO 队列、分段发送、reader task 生命周期、/interrupt 指令、session resume |
